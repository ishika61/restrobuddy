...
export default function NavBarSearchBarComponent({oldsearch})
{
    var theme=useTheme()
  ...
  if (
    ...
    const ShowSugetionsList=()=>{
        ...
        return(<div style={{position:'fixed',top:dropdownPos.top,left: dropdownPos.left,width: dropdownPos.width,maxHeight:230,background:'#f9f9f9',border:'1px solid #ccc',overflowY:'auto',borderRadius:5,zIndex:99999}}>
    ...
    return(
    <div style={{padding:'1.6%',background:'#fff',width:matches? '50%':'100%',height:matches? '3vh':'5vh',borderRadius:5,boxShadow: 'rgba(0, 0, 0, 0.08) 0px 0px 12px',display:'flex',alignItems:'center'}}>
        <div>
            <img src={location} style={{objectFit:'contain',maxWidth:'1.4rem'}} />
        </div>
        <div>
        ...
        <div>
            <img src={search} style={{objectFit:'contain',maxWidth:'1.4rem',marginLeft:'30%'}} />
        </div>
        <div>
            <input ref={inputRef} value={searchValue} onChange={handleSeachValue} onKeyDown={handleKeyDown} type='text' style={{width:matches? '20vw':'30vw',height:30,outline:'none',border:0,fontSize:14,color:'grey',marginLeft:10}} placeholder={matches?"Search for restaurant...":"Search for restaurant, cuisine or a dish"} />
        </div>
         {ShowSugetionsList()}
    </div>)
}