...
export default function LoginShopkeeper()
{   const classes=useStyles()
    ...
    const handleSubmit=async()=>{
        ...
        var res=await postData('restaurant/check_resadmin_login',body)
        if(res.status)
        {
            localStorage.setItem("RESADMIN",JSON.stringify(res.data))
            navigate("/shopkeeperdashboard",{state:res.data})
        }
        else
        {
            swal.fire({
